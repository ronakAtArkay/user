...
def get_user_by_id(user_id: str, db: Session):
    return db.query(UserModel).filter(UserModel.id == user_id, UserModel.is_deleted == False).first()
...
def get_user_list(start:int, limit:int, sort_by: str, order:str, search: str, db: Session):
...
def get_all_users(db: Session):
    db_user = db.query(UserModel).filter(UserModel.is_deleted == False).order_by(UserModel.name.desc()).all()
    return db_user
